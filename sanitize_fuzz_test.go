...
// FuzzFirstToUpper_General validates that FirstToUpper capitalizes the first character while leaving the rest untouched.
...
// FuzzFormalName_General validates that FormalName only returns characters typically allowed in proper names.
...
// FuzzIPAddress_General validates that IPAddress returns a canonical IP string when input contains a valid address.
